...
cat << EOF
         ...
         ${BLUE}%%%%%%%%%%%%&&&${NORMAL}#  ${BLUE}down1x${NORMAL}  #${GREEN}%&&&&%%%%%%%%%%%%${NORMAL}
     ${BLUE}#######((((###%%%%%${NORMAL}#############${GREEN}&%%%%%%%${NORMAL}
     ${BLUE}######/     ########%%%%&&&&&&&%%${NORMAL}
      ${BLUE}((((((((((((######%%%%%%%${NORMAL}
       ${BLUE}(((((((((#####%%*${NORMAL}
...
DOWNRA1N='https://github.com/edwin170/downr1n.git'
...
# kernel modules
cat << ! > rootfs/etc/mkinitfs/features.d/down1x.modules
...
chroot rootfs /usr/bin/env PATH=/usr/bin:/bin:/usr/sbin:/sbin:/usr/local/bin \
	/sbin/mkinitfs -F "down1x" -k -t /tmp -q $(ls rootfs/lib/modules)
...
echo 'down1x' > rootfs/etc/hostname
echo "PATH=$PATH:$HOME/.local/bin" > rootfs/root/.bashrc # d
echo "export DOWN1X_VERSION='$VERSION'" > rootfs/root/.bashrc
